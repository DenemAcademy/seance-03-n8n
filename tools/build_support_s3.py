...
CHAPTERS = [
    (
        ...
        "On pose les mots simples avant de toucher au canvas. Le point important de cette séance : n8n est dense, son apprentissage prend du temps, donc on cherche d’abord à comprendre la logique générale pour ensuite guider Codex ou Claude Code.",
        "Comprendre",
        "blue",
        [
            item(
                "n8n en phrase simple",
                "n8n est un outil d’automatisation visuelle. Il relie des services entre eux : formulaire, email, Slack, Notion, CRM, API, IA. Une action arrive, n8n la reçoit, applique une logique, puis déclenche la suite. C’est utile quand un client veut une automatisation visible, contrôlable et modifiable sans cacher tout le travail dans du code.",
                ...
                "user-n8n/01-overview-workflows.png",
                link="n8n_docs",
            ...
            item(
                ...
                "Automatiser ne veut pas dire tout rendre automatique. Une bonne automatisation enlève une action répétitive, pas une décision importante. n8n reste un outil puissant, mais il peut vite devenir compliqué si le besoin n’est pas découpé. Le but de la séance est donc de comprendre le chemin : déclencheur, donnée, nodes, sortie, preuve.",
                "Séparez les tâches en trois groupes : ce qui peut se faire seul, ce qui doit être préparé par n8n, et ce qui doit rester validé à la main. Cette séparation évite les workflows trop dangereux et donne un brief clair à Codex ou Claude Code.",
                ...
                "user-n8n/02-workflow-editor.png",
            ...
            item(
                ...
                "Toute la séance suit le même exemple. Le Comptoir Bleu reçoit une demande depuis un formulaire : nom, email, date, nombre de personnes et message libre. n8n doit nettoyer la donnée, vérifier les champs importants, préparer une réponse, notifier l’équipe et garder une trace. Ce cas simple permet de comprendre les bases sans partir dans tous les sens.",
                ...
                "user-n8n/03-execution-success.png",
                link="n8n_workflows",
            ...
            item(
                ...
                "user-n8n/07-add-node-categories.png",
                link="n8n_workflow_components",
                code=["Workflow = histoire complète", "Trigger = départ", "Node = étape", "Execution = preuve du passage"],
            ),
            item(
                "La donnée circule en JSON",
                "n8n transporte des données sous forme de JSON. Dit simplement, le JSON est une façon de ranger des informations avec des noms de champs et des valeurs. Ce n’est pas juste du texte sur un écran. Un node reçoit des champs, les modifie, puis transmet une sortie. Comprendre ça évite de chercher une erreur au mauvais endroit.",
                "Quand un node se termine, ouvrez sa sortie et repérez les noms exacts des champs : `name`, `email`, `message`, `date`. Le node suivant doit utiliser ces noms exacts. Codex ou Claude Code peut aider à écrire le JSON, mais vous devez savoir le relire.",
            ...
            item(
                ...
                "user-n8n/14-instance-mcp.png",
                link="n8n_mcp_instance",
                code=["Node = brique n8n", "API = appel HTTP", "MCP = outil exposé à un agent"],
            ),
            item(
                "La place de Claude Code et Codex",
                "Claude Code et Codex ne sont pas des nodes n8n classiques. On ne les met pas dans le canvas comme un simple bouton. Ils aident surtout à créer, relire et corriger la structure : workflow JSON, logique des nodes, scripts API, ou outils MCP. C’est le vrai centre de la séance.",
                "Retenez la phrase : `Claude Code et Codex ne remplacent pas la compréhension de n8n, ils accélèrent la construction quand le besoin est clair.` Ils peuvent créer, modifier, relire ou déclencher des workflows si les accès sont bien cadrés.",
    ...
    (
        ...
        [
            ...
            item("Webhook pour recevoir", "Le Webhook permet à un service externe d’envoyer une donnée à n8n. Il est utile pour formulaires, landing pages, CRM, outils no-code ou scripts. Le webhook est une porte d’entrée.", "Ajoutez un Webhook Trigger et copiez son URL de test. Envoyez une donnée simple avant de brancher un vrai outil.", "L’execution montre que n8n reçoit bien le payload.", "Est-ce que la donnée arrive vraiment, ou est-ce que je regarde seulement l’URL ?", "user-n8n/09-search-trigger.png", link="n8n_workflow_components", code=["POST https://votre-n8n/webhook-test/lead-comptoir-bleu"]),
    ...
    (
        ...
        "violet",
        [
            item("OpenAI et Anthropic dans n8n", "n8n peut appeler des modèles via des nodes IA ou HTTP Request. C’est différent de Codex ou Claude Code. Ici, n8n parle à une API de modèle pour produire un texte, classer une demande ou extraire une information.", "Utilisez un credential dédié pour OpenAI ou Anthropic. Le prompt doit être court, précis, et relié aux champs du workflow.", "La réponse du modèle est utilisée comme une donnée, pas comme une vérité automatique.", "Est-ce que l’IA produit une aide ou une action finale ?", "user-n8n/05-openai-credential.png", link="n8n_ai_agent"),
      ...
      <div class="mx-auto grid min-h-[82vh] max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1fr_.9fr] lg:items-center lg:px-8">
        <div>
          ...
          <p class="mt-6 max-w-2xl text-xl leading-9 text-slate-700">
            Dans cette séance, on part de zéro : ce qu’est n8n, pourquoi on l’utilise quand un client veut une automatisation visuelle, comment lire les nodes, comment comprendre le JSON, puis comment connecter Claude Code et Codex avec MCP ou avec l’API n8n.
          </p>
          <p class="mt-5 max-w-2xl text-lg leading-8 text-slate-700">
            Le point important : n8n reste un outil dense. L’objectif n’est pas de tout connaître par cœur dès maintenant. L’objectif est de comprendre assez bien le fonctionnement pour demander ensuite à Codex ou Claude Code de créer, corriger et expliquer un workflow propre.
          </p>
          ...
          <div class="mt-6 grid gap-3 sm:grid-cols-2">
            <div class="border-2 border-slate-950 bg-white p-4 shadow-neo-sm">
              <p class="font-mono text-[11px] font-black uppercase tracking-[.14em] text-blue-700">1 · Comprendre</p>
              <p class="mt-2 text-sm font-semibold leading-6 text-slate-700">Pourquoi n8n sert à montrer un processus client étape par étape.</p>
            </div>
            <div class="border-2 border-slate-950 bg-white p-4 shadow-neo-sm">
              <p class="font-mono text-[11px] font-black uppercase tracking-[.14em] text-violet-700">2 · Nodes</p>
              <p class="mt-2 text-sm font-semibold leading-6 text-slate-700">Trigger, action, logique, IA, erreur : chaque node a un rôle simple.</p>
            </div>
            <div class="border-2 border-slate-950 bg-white p-4 shadow-neo-sm">
              <p class="font-mono text-[11px] font-black uppercase tracking-[.14em] text-cyan-700">3 · JSON</p>
              <p class="mt-2 text-sm font-semibold leading-6 text-slate-700">Lire les champs qui entrent et sortent avant de corriger un workflow.</p>
            </div>
            <div class="border-2 border-slate-950 bg-white p-4 shadow-neo-sm">
              <p class="font-mono text-[11px] font-black uppercase tracking-[.14em] text-emerald-700">4 · Agents</p>
              <p class="mt-2 text-sm font-semibold leading-6 text-slate-700">API pour créer/modifier, MCP pour utiliser des outils exposés.</p>
            </div>
          </div>
        </div>
